...
# --- Initialize LangChain (Model and Static Chain Components) ---
# Only initialize the static components here
try:
    llm = Ollama(model=MODEL_NAME)
    # Static prompt template (used for formatting, not determining system prompt)
    prompt_template = ChatPromptTemplate.from_messages([
        MessagesPlaceholder(variable_name="history"), # Placeholder for history including System msg
        ("user", "{input}")
    ])
    output_parser = StrOutputParser()
    # The actual runnable chain (prompt | llm | parser) will be constructed dynamically per request if needed,
    # or we can use llm.stream directly with the constructed message list.
    print(f"Successfully initialized LangChain base components with model: {MODEL_NAME}")
except Exception as e:
    print(f"Error initializing LangChain base components: {e}")
    llm = None
...
def chat():
    if llm is None or prompt_template is None or output_parser is None:
         def error_stream(): # Error before processing starts
            error_data = json.dumps({"error": "Chatbot not fully initialized. Check backend logs."})
    ...
    try:
        data = request.json
        user_message_original = data.get('message') # Keep original for history/fallback
        ...
        # --- Normal LLM Processing --- (This part remains unchanged)
        if should_fallback_to_llm:
            print(f"[app.py /chat] Proceeding with standard LLM call.")
            # ... (Determine system prompt, convert history, define generate_response) ...
            effective_rules = ""
            ...
            for history_entry in history_tuples:
                ...
                try:
                    ...
                    entry_len = len(history_entry)
                    if role == "user" and entry_len == 2: message_object = HumanMessage(content=content)
                    elif role == "assistant" and entry_len >= 2: message_object = AIMessage(content=content) 
                    else: continue
                    if message_object: history_messages.append(message_object)
            ...
            def generate_response():
                messages_for_llm = [
                    ...
                    *history_messages,
                    HumanMessage(content=user_message_for_llm) # Use potentially modified message
                ]
                # ... (logging messages for LLM) ...
                print(f"\n[app.py /chat generate_response] Messages passed to LLM:")
                for msg in messages_for_llm: print(f"  - {type(msg).__name__}: {msg.content[:100]}...")
                print("------")

                try:
                    # ... (LLM stream call and yield loop) ...
                    print("[app.py /chat generate_response] Attempting LLM stream...")
                    stream_iterator = llm.stream(messages_for_llm)
                    for chunk in stream_iterator:
                        chunk_data = json.dumps({"chunk": chunk})
                        yield f"data: {chunk_data}\n\n"
                    print("[app.py /chat generate_response] LLM Stream finished.")
                    yield f"event: end\ndata: finished\n\n"
                except Exception as e:
                    # ... (LLM stream error yielding) ...
                    print(f"[app.py /chat generate_response] Error during LLM stream: {e}")
                    error_data = json.dumps({"error": "Error generating response stream."})
                    yield f"data: {error_data}\n\n"
                    yield f"event: error\ndata: finished\n\n"
...
def react():
    ...
    if llm is None:
        print("[app.py /react] LLM not initialized.")
        # Return error as SSE
        def error_stream():
            error_data = json.dumps({"error": "Chatbot not initialized."})
            yield f"data: {error_data}\n\n"
            yield f"event: error\ndata: finished\n\n"
    ...
    try:
        ...
        bot_persona = data.get('botPersona')
        ...
        # --- Determine system prompt (same as before) ---
        ...
        def generate_ack_stream():
            try:
                print("[app.py /react generate_ack_stream] Prompting LLM for acknowledgment stream...")
                # Use llm.stream for the acknowledgment
                stream_iterator = llm.stream(messages_for_react)

                for chunk in stream_iterator:
                    chunk_data = json.dumps({"chunk": chunk})
                    yield f"data: {chunk_data}\n\n"
                
                print("[app.py /react generate_ack_stream] Acknowledgment stream finished.")
                yield f"event: end\ndata: finished\n\n"

            except Exception as e:
                print(f"[app.py /react generate_ack_stream] Error during stream generation: {e}")
                error_data = json.dumps({"error": "Error generating acknowledgment stream."})
                yield f"data: {error_data}\n\n"
                yield f"event: error\ndata: finished\n\n"
...
if __name__ == '__main__':
    ...
    # Run the Flask app
    # Note: use_reloader=False would also fix this, but disables hot-reloading
